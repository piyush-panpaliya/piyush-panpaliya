...
export const Project = ({ name, link }: { name: string; link: string }) => {
  ...
  return (
    ...
    >
      ...
      <div className={cn(path == link && 'text-white')}>{name}</div>
    </Link>
  )
}
...
const Projects = () => {
  ...
  return (
    <div>
      ...
      {show && (
        <div className="devmb-4 flex flex-col px-4 py-2">
          <Project name="all" link="/projects" />
        </div>
      )}
    </div>
  )
